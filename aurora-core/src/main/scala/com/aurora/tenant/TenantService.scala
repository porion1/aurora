 ...
 * Fully integrated with TenantConfigService for per-tenant configuration
 */
object TenantService {
  ...
  private def ensureIndex(keys: Bson, options: IndexOptions, tenantId: String, dedicated: Boolean): Unit = {
    Try(collection(tenantId, dedicated).createIndex(keys, options)) match {
      case Success(indexName) => logInfo(s"Index created/verified: $indexName")
      case Failure(ex) if ex.getMessage.contains("already exists") =>
        logInfo(s"Index already exists: ${options.getName}")
      case Failure(ex) => logWarn(s"Index creation warning: ${ex.getMessage}")
    }
  }
  ...
  def updateTenantName(id: String, newName: String): Try[Tenant] = Try {
    ...
    getTenantByName(newName).get.foreach { existing =>
      if (existing.id != id) throw new IllegalStateException(s"Tenant with name '$newName' already exists")
    }
    ...
    Option(collection(tenantId, dedicated).findOneAndUpdate(filter, update, options))
      .map(toTenant)
      .getOrElse(throw new IllegalStateException(s"Tenant with id $id not found or inactive"))
  }
  ...
  def updateTenantStatus(id: String, isActive: Boolean): Try[Tenant] = Try {
    ...
    val filter = Filters.and(Filters.eq("tenantId", tenantId), Filters.eq("id", id))
    val update = Updates.combine(
    ...
    Option(collection(tenantId, dedicated).findOneAndUpdate(filter, update, options))
      .map(toTenant)
      .getOrElse(throw new IllegalStateException(s"Tenant with id $id not found"))
  }
  ...
  def migrateCurrentTenantToDedicated(): Try[Boolean] = {
    ...
    val migrationResult = TenantDatabaseManager.migrateToDedicated(tenantId)

    // -------------------------
    // NEW: Migrate tenant config to dedicated DB
    // -------------------------
    import TenantConfigService.*
    getConfig(tenantId) match {
      case Success(config) =>
        updateConfig(config.copy(tenantId = tenantId))
        logInfo(s"Tenant config migrated to dedicated DB for $tenantId")
      case Failure(ex) =>
        logWarn(s"No tenant config found to migrate for $tenantId: ${ex.getMessage}")
    }
    // -------------------------
  ...
  // -------------------------
  // HELPER: Fetch tenant config
  // -------------------------
  def getTenantConfig(tenantId: String = TenantContext.getCurrentTenantId): Try[TenantConfig] =
    TenantConfigService.getConfig(tenantId)

  // -------------------------
  // VALIDATION
  // -------------------------
  private def validateTenantName(name: String): Unit = {
    if (name == null || name.trim.isEmpty) throw new IllegalArgumentException("Tenant name cannot be empty")
    if (name.length > 100) throw new IllegalArgumentException("Tenant name cannot exceed 100 characters")
  }
  ...
  private def activeTenantFilter(tenantId: String): Bson =
    Filters.and(Filters.eq("tenantId", tenantId), Filters.eq("isActive", true))

  private def activeTenantByIdFilter(tenantId: String, id: String): Bson =
    Filters.and(Filters.eq("tenantId", tenantId), Filters.eq("id", id), Filters.eq("isActive", true))

  // -------------------------
  // MAPPERS
  // -------------------------
  private def toDocument(t: Tenant): Document = new Document()
    .append("id", t.id)
  ...
  private def toTenant(doc: Document): Tenant = Tenant(
    id = doc.getString("id"),
  ...
  // ==============================
  // GET TENANT BY NAME (ACTIVE ONLY)
  // ==============================
